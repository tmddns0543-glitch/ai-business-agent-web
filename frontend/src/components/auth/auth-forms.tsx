...
export function SignupForm() {
  ...
  return <form action={action} className="space-y-5"><AuthField id="signup-email" name="email" label="이메일" type="email" autoComplete="email" /><AuthField id="signup-password" name="password" label="비밀번호" type="password" autoComplete="new-password" /><AuthField id="signup-password-confirmation" name="passwordConfirmation" label="비밀번호 확인" type="password" autoComplete="new-password" /><p className="text-xs leading-5 text-slate-500">비밀번호는 8자 이상으로 입력하세요. 실제 허용 조건은 Supabase 프로젝트의 비밀번호 정책을 따릅니다.</p><StatusMessage state={state} /><SubmitButton pending={pending} idle="회원가입" loading="가입 처리 중..." /></form>;
}
