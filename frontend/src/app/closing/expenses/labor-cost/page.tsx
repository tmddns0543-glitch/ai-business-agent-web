...
import {
  calculateLaborCostInputSummary,
  type LaborCostInputValues,
} from "@/lib/expense/calculate-labor-cost-input-summary";
...
function createEditableItems(
  ...
  const activeItems = getExpenseItemsForGroup("labor-cost");
  const allCustomItems = getExpenseCustomItemsByGroup("labor-cost", {
...
export default function LaborCostPage() {
  ...
  const [values, setValues] = useState<LaborCostInputValues>({});
  ...
  const inputTotal = useMemo(
    () => calculateLaborCostInputSummary(values),
    [values],
  );
  ...
  function saveLaborCosts() {
    ...
    if (
      ...
      reload(businessDate);
      setMessage("인건비를 저장했습니다.");
    } else {
      setError(
        "인건비를 저장하지 못했습니다. 다시 시도해주세요.",
      );
    }
          ...
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-2xl font-bold text-indigo-700">
                {formatMoney(storedSummary.byGroup["labor-cost"])}
              </p>
            </div>
