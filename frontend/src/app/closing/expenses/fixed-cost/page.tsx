...
import {
  calculateFixedCostInputSummary,
  type FixedCostInputValues,
} from "@/lib/expense/calculate-fixed-cost-input-summary";
...
function createEditableItems(
  ...
  const activeItems = getExpenseItemsForGroup("fixed-cost");
  const allCustomItems = getExpenseCustomItemsByGroup("fixed-cost", {
...
export default function FixedCostPage() {
  ...
  const [values, setValues] = useState<FixedCostInputValues>({});
  ...
  const inputTotal = useMemo(
    () => calculateFixedCostInputSummary(values),
    [values],
  );
  ...
  function saveFixedCosts() {
    ...
    if (
      ...
      reload(businessDate);
      setMessage("공과금·고정비를 저장했습니다.");
    } else {
      setError(
        "공과금·고정비를 저장하지 못했습니다. 다시 시도해주세요.",
      );
    }
          ...
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-2xl font-bold text-indigo-700">
                {formatMoney(storedSummary.byGroup["fixed-cost"])}
              </p>
            </div>
