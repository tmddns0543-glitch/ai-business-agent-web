...
import {
  calculateOperatingCostInputSummary,
  type OperatingCostInputValues,
} from "@/lib/expense/calculate-operating-cost-input-summary";
...
function createEditableItems(
  ...
  const activeItems = getExpenseItemsForGroup("operating-cost");
  const allCustomItems = getExpenseCustomItemsByGroup("operating-cost", {
...
export default function OperatingCostPage() {
  ...
  const [values, setValues] = useState<OperatingCostInputValues>({});
  ...
  const inputTotal = useMemo(
    () => calculateOperatingCostInputSummary(values),
    [values],
  );
  ...
  function saveOperatingCosts() {
    ...
    if (
      ...
      reload(businessDate);
      setMessage("일반 운영비를 저장했습니다.");
    } else {
      setError(
        "일반 운영비를 저장하지 못했습니다. 다시 시도해주세요.",
      );
    }
          ...
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-2xl font-bold text-indigo-700">
                {formatMoney(storedSummary.byGroup["operating-cost"])}
              </p>
            </div>
