...
export default function GeneralSettingsPage() {
  ...
  const saveFlow = useFeeSettingSaveFlow({ platformId: "general", onMessage: setMessage, onSaved: () => { setCardRate(readCardRate()); setError(undefined); setMessage("일반결제 설정을 저장했습니다."); } });
  ...
  function save(event: FormEvent<HTMLFormElement>) {
    ...
    saveFlow.requestSave(createPlatformFeeSettings(getBusinessFeeSettings(), "general", { [CHANNEL_IDS.card]: { cardRate: normalized } }));
  }

  function restore() {
    if (!window.confirm("일반결제 설정을 기본값으로 되돌릴까요?")) return;
    saveFlow.requestSave(createPlatformFeeSettings(getBusinessFeeSettings(), "general", Object.fromEntries(Object.values(CHANNEL_IDS).map((channelId) => [channelId, getDefaultChannelUpdate(channelId)]))));
  }

  if (cardRate === null) return <SettingsLoading />;
  return <SettingsPageLayout title="일반결제 설정" description="매장에서 직접 결제되는 매출의 공제 기준을 설정하세요."><form onSubmit={save} noValidate><div className="space-y-4"><SettingChannelCard title="카드결제"><FeeRateField id="generalCardRate" label="카드수수료율" value={cardRate} max={1.5} error={error} onChange={(value) => { setCardRate(value); setError(undefined); setMessage(""); }} /></SettingChannelCard><SettingChannelCard title="현금결제" description="현재 기본 공제 항목이 없습니다." /><SettingChannelCard title="계좌이체" description="현재 기본 공제 항목이 없습니다." /></div>{saveFlow.status}<SettingsFormActions message={message} onRestore={restore} /></form>{saveFlow.dialogs}</SettingsPageLayout>;
}
